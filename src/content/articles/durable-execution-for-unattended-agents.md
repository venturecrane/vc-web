...
date: 2026-06-21
description: 'An agent task that exceeds a single turn budget needs durable, resumable, cancellable execution: a job ledger and a segmented worker, not a bigger context window.'
...
The substrate has four parts, and none of them is a new persistence engine. The design principle was to leverage primitives that already existed rather than build a general durable-execution framework.
...
An **in-process worker thread** advances jobs in segments. This is the part that matters most and is easiest to get wrong. The worker is not a new replicated process and not an async task on the request-handling event loop. It is a background thread inside the gateway that already runs scheduled work, reusing the same agent-construction path the cron scheduler uses. It claims a job, resumes the conversation from the recorded tip, runs one segment of work, records what it spent and where it left off, and loops. The job advances across many segments and many turns. No single segment has to fit the whole task inside one reply budget.
