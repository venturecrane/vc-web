...
**Stale secrets.** Someone rotates an API key. The `.env` file on three machines still has the old value. Nobody notices until an agent session fails mid-task, and the error message points to an authentication failure that could mean a dozen things.

**Wrong-project injection.** Copy a `.env` from one project to another. Change four of seven keys. Miss the fifth. The agent runs with a hybrid environment - partially project A, partially project B - and produces behavior that's subtly wrong in ways that are hard to diagnose.
...
We covered these failure modes in [the injection article](/articles/secrets-injection-agent-launch). The solution is a centralized secrets manager with runtime injection. The rest of this article is about the organizational layer on top of that: how to structure, separate, and selectively expose secrets when you're running multiple projects, multiple environments, and autonomous agents that should only see what they need.
...
└── /epsilon  - Project Epsilon secrets
```

The CLI launcher knows which path maps to which project. When you type `crane alpha`, it fetches secrets from `/alpha`, injects them as environment variables, and spawns the agent session. One command, one fetch, no files on disk.

Shared secrets - infrastructure keys that every project needs - live at a designated source path. A sync script propagates them to every other path. The source of truth is always one place. When a shared key gets rotated, you update it once and run the sync. No manual copy-paste across five project paths.
...
CRANE_ENV=dev crane alpha  # Dev/staging secrets
```

Both environments exist in the secrets manager at the same project path, just in different environment scopes (Infisical's native concept). The launcher fetches from the correct environment and injects `CRANE_ENV` itself so the running agent knows which context it's operating in.
...
The script prompts for Machine Identity credentials, writes the credentials file with restricted permissions, and verifies authentication works. After that, `crane alpha` works identically whether you're at the keyboard or SSH'd in from a tablet across the country.
